...
3. **Compose ONE single Slack message** containing both papers. The full
   ...
   **"이런 각도/사례도 있더라"고 연구자 시야를 넓혀드리는 큐레이션**의 성격. 따라서:
   ...
   - `proteogenomics × 항암제 반응성 영역에서 참고할 만한 논문 N편을 추려보았습니다.`
   - `오늘은 단백유전체-항암제 반응성 주제의 논문 N편을 정리해보았습니다.`
   - `참고하실 만한 단백유전체 × 항암제 반응성 논문 N편을 모아보았습니다.`
   - `proteogenomics·phosphoproteome 기반 종양 연구에서 N편을 살펴보았습니다.`
   - `암 단백유전체 및 치료 반응성 관련 최근 논문 N편을 짧게 정리했습니다.`

   해당 날짜의 논문 주제(예: 신생항원 위주, ppQTL 위주, 임상 trial 위주)에 맞게 표현을 살짝 조정해도 됨.
