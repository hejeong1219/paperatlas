...
**디지스트의 성격**: 봇이 자동으로 큐레이션해서 전달하는 정보 소스. "이걸 보고 당장
적용하세요"가 아니라, "이런 각도/사례도 있더라"고 **연구자 시야를 넓혀드리는 큐레이션**.
정보 소스를 정중히 던져드리는 톤.

- 청중: 코딩하지 않는 임상·기초 교수진. 정중하고 상냥하게.
- 격식 있는 보고체 (~합니다, ~입니다, ~보입니다, ~사료됩니다)
- **인트로/부제 추가 금지**: 헤더(오늘의 참고 논문) 다음 빈 줄 후 바로 논문으로 진입.
- **명령·지시 어조 금지**: "활용 가능합니다", "사용해야 합니다", "적용해야 합니다" 등 ❌
- **가능성·시사·참고 어조**: "~참고해볼 수 있겠습니다", "~시사합니다", "~보입니다" ✅
- 영어 논문 제목·저자명·기관명은 그대로
- 영어 기술 용어 그대로 유지 (proteogenomics, phosphoproteome, neoantigen, ppQTL 등 번역 X)
- 약자 처음 등장 시 풀어쓰기 (예: WGS(전장유전체))
- "활용 line"의 **앵글을 다양화** — 매일 "파이프라인 참고"로만 매핑하면 청중이 지루함. 5가지 앵글 중 논문 내용에 가장 자연스러운 것:
  * (A) 생물학적 발견: 논문의 특정 단백질·유전자·기전 → 한미암 데이터에서 검토 후보
  * (B) 바이오마커·환자 층화: 마커 후보 제시 → 한미암 코호트 층화에 고려
  * (C) 표적·치료 경로: 새 표적·병용 가설 → 한미암 면역/표적치료 분석 연관성
  * (D) 코호트·디자인: 종단·비교 디자인 → 한미암 1차/2차 내성 비교 시사점
  * (E) 메서드·파이프라인: 분석 워크플로 → 한미암 파이프라인 구축 참고
- 발견 위주 논문이면 (A)/(B), 임상·치료 위주면 (C), 종단 디자인이면 (D), 순수 메서드면 (E). 메서드(E)만 매일 나오지 않게 의식적으로 다양화.
- 새로운 도구·개념을 그냥 던지지 말고, 본 과제의 어느 단계·어느 분석에 어떻게 연결되는지로 변환 — 단, 변환 결과를 명령형이 아닌 시사형으로

## 게시 포맷 템플릿 (Slack — 1 message per day, 2 papers, 3 lines per paper)
...
**인트로/부제 추가 금지**. 헤더 → 빈 줄 → 바로 첫 논문 citation.
