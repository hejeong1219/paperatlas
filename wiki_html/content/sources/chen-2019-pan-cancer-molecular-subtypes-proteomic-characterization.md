---
title: Pan-cancer molecular subtypes revealed by mass-spectrometry-based proteomic characterization of more than 500 human cancers
authors:
  - "Chen"
year: 2019
journal: "Nature Communications"
doi: "10.1038/s41467-019-13528-0"
url: "https://www.nature.com/articles/s41467-019-13528-0"
pdf: "raw/inbox/papers/chen-2019-pan-cancer-molecular-subtypes-proteomic-characterization.pdf"
paper_kind: cohort-study
cancer_types:
  - pan-cancer
modalities:
  - proteomics
  - phosphoproteomics
  - pan-cancer-subtyping
themes:
  - cptac
  - proteome-subtypes
  - pathway-stratification
  - humoral-immune-subtype
tags:
  - source
  - pan-cancer
  - proteomics
  - phosphoproteomics
  - cptac
topic: ptmanchor
extra_topics:
  - "bcell-neoantigen"
batch_ingest_status: pdf-text-extracted
batch_ingested_on: 2026-05-13
---
# Pan-cancer molecular subtypes revealed by mass-spectrometry-based proteomic characterization of more than 500 human cancers

Landmark CPTAC-centered pan-cancer study showing that proteomic and phosphoproteomic profiling can define cross-lineage molecular subtypes not fully captured by transcriptomics.

## Batch PDF Ingest Status

- Status: `pdf-text-extracted` on 2026-05-13.
- Local PDF: `raw/inbox/papers/chen-2019-pan-cancer-molecular-subtypes-proteomic-characterization.pdf`.
- Extracted text length: 60,085 characters.
- Scope note: automated local-PDF text ingest for the 100-paper drug-response/phospho-global batch; not yet a manual full-text deep-dive.
- Evidence boundary: downstream scientific claims should use this page only after source-specific Key Points are manually promoted or rechecked against the local PDF.
- High-signal PDF snippets:
  - ARTICLE https://doi.org/10.1038/s41467-019-13528-0 OPEN Pan-cancer molecular subtypes revealed by mass- spectrometry-based proteomic characterization of more than 500 human cancers Fengju Chen1, Darshan S.
  - Creighton 1,5,6,7* 1234567890():,; Mass-spectrometry-based proteomic proﬁling of human cancers has the potential for pan- cancer analyses to identify molecular subtypes and associated pathway features that might be otherwise missed using transcriptomics.
  - The proteome-based subtypes are obser- vable in external cancer proteomic datasets surveyed.
  - common exercises utilizing transcriptomic or proteomic data Mass-spectrometry-based proteomic-proﬁling data of human on human tumors.
  - We also examine cancer have involved transcriptomic rather than proteomic data, as the proteomic and transcriptomic datasets outside of CPTAC, for advent of DNA microarrays over 20 years ago began the wide- patterns of manifestation of our proteome-based subtypes.

## Key Points

- Analyzes 532 human cancers across multiple tissue types using mass-spectrometry-based proteomic profiling, with phosphoproteomic features incorporated into subtype characterization.
- Defines ten proteome-based pan-cancer subtypes that cut across tumor lineage and reveal pathway structure missed by transcriptomics alone.
- Identifies immune-related and stromal-related subtypes, including one proteome-defined subtype associated with humoral immune response.
- Shows that pan-cancer proteomic organization can recover clinically and biologically meaningful structure from human tumor cohorts.
- Provides an older but still foundational benchmark for cross-cohort human cancer proteomics and phosphoproteomics.

## Relevance

- Useful for the ptmanchor manuscript as a landmark human-cancer cohort reference showing why protein and phosphoprotein layers deserve interpretation beyond transcript-level summaries.
- Supports the broader framing that abundance-aware protein and PTM analysis can reveal biology that standard RNA-based classification underestimates.
- Also marginally relevant to the B-cell neoantigen side because one subtype centers humoral immunity, but its primary value here is as foundational pan-cancer proteomics background.

## Connections

- [ptmanchor Manuscript Anchor](../analyses/ptmanchor-manuscript-anchor.md)
- [PTM Correction and Kinase Signaling in Cancer Proteomics](../topics/ptm-correction-and-kinase-signaling-in-cancer-proteomics.md)
- [Pan-Cancer](../cancers/pan-cancer.md)
- [Mapping the functional network of human cancer through machine learning and pan-cancer proteogenomics](./shi-2025-functional-network-human-cancer-proteogenomics.md)

## Sources

- PDF: [chen-2019-pan-cancer-molecular-subtypes-proteomic-characterization.pdf](../../raw/inbox/papers/chen-2019-pan-cancer-molecular-subtypes-proteomic-characterization.pdf)
- Article: <https://www.nature.com/articles/s41467-019-13528-0>
